...
class PackIt_Admin {
    ...
    public function enqueue_scripts($hook) {
        ...
        wp_enqueue_style(
            'packit-admin-style',
            PACKIT_PLUGIN_URL . 'assets/css/packit-admin.css',
            array(),
            '1.0.0'
        );
        ...
        wp_enqueue_script(
            'packit-admin-script',
            PACKIT_PLUGIN_URL . 'assets/js/packit-admin.js',
            array('jquery'),
            '1.0.0',
            true
        );
    ...
    public function handle_export_requests() {
        ...
        if (!isset($_GET['_wpnonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_GET['_wpnonce'])), 'packit_export')) {
            return;
        }
    ...
    public function admin_page() {
        ...
        ?>
        <div class="wrap packit-admin-page">
            <h1><?php esc_html_e('Exportador de Plugins e Temas', 'packit-plugin-theme-exporter'); ?></h1>
            
            <div class="packit-tabs">
                <a href="#" class="packit-tab active" data-target="plugins-content"><?php esc_html_e('Plugins', 'packit-plugin-theme-exporter'); ?></a>
                <a href="#" class="packit-tab" data-target="themes-content"><?php esc_html_e('Temas', 'packit-plugin-theme-exporter'); ?></a>
            </div>
            
            <div class="packit-search-box">
                <input type="text" id="packit-search" placeholder="<?php esc_attr_e('Pesquisar...', 'packit-plugin-theme-exporter'); ?>">
            </div>
            
            <div id="plugins-content" class="packit-tab-content active">
                <h2><?php esc_html_e('Plugins Instalados', 'packit-plugin-theme-exporter'); ?></h2>
                <table class="packit-table">
                    <thead>
                        <tr>
                        ...
                        <tr>
                            ...
                            <td>
                                <?php if ($plugin['active']): ?>
                                    <span class="packit-status-active"><?php esc_html_e('Ativo', 'packit-plugin-theme-exporter'); ?></span>
                                <?php else: ?>
                                    <span class="packit-status-inactive"><?php esc_html_e('Inativo', 'packit-plugin-theme-exporter'); ?></span>
                                <?php endif; ?>
                            </td>
                            <td>
                                <a href="<?php echo esc_url(wp_nonce_url(admin_url('tools.php?page=packit-plugin-theme-exporter&export_plugin=' . urlencode($plugin['path'])), 'packit_export')); ?>" class="packit-export-button" data-item-name="<?php echo esc_attr($plugin['name']); ?>" data-item-type="plugin">
            ...
            <div id="themes-content" class="packit-tab-content">
                ...
                <table class="packit-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <a href="<?php echo esc_url(wp_nonce_url(admin_url('tools.php?page=packit-plugin-theme-exporter&export_theme=' . urlencode($theme['path'])), 'packit_export')); ?>" class="packit-export-button" data-item-name="<?php echo esc_attr($theme['name']); ?>" data-item-type="theme">
                                    <?php esc_html_e('Exportar', 'packit-plugin-theme-exporter'); ?>
                                </a>
                            </td>
                        </tr>
